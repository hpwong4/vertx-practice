...
public class CollectorService extends AbstractVerticle {
  ...
  public void start(Promise<Void> startPromise) throws Exception {
    ...
    vertx.createHttpServer().requestHandler(this::handleRequest).listen(8080)
        .onFailure(startPromise::fail).onSuccess(ok -> {
  ...
  private Future<JsonObject> sendToSnapshot(CompositeFuture temps) {
    ...
    JsonObject data = new JsonObject()
        .put("data", new JsonArray().add(tempData.get(0)).add(tempData.get(1)).add(tempData.get(2)));

    return webClient
        .post(4000, "localhost", "/")
        .expect(ResponsePredicate.SC_SUCCESS)
        .sendJson(data).map(response -> data);
  }
